...
(function(window, undefined) {
    ...
    "createSVGShape" : function($shape) {
            ...
            if (shapeSVG != null) {
                ...
                shapeSVG.css2svg = {
                    ...
                    "original-width":  ($shape.data("originalWidth")) ? parseInt($shape.data("originalWidth")) : parseInt($shape.css("width")),
                    "original-height":  ($shape.data("originalHeight")) ? parseInt($shape.data("originalHeight")) : parseInt($shape.css("height")),
                    "original-width-ratio": ($shape.data("originalWidth")) ? parseFloat($shape.data("originalWidth") / parseInt($shape.css("width"))) : 1,
                    "original-height-ratio": ($shape.data("originalHeight")) ? parseFloat($shape.data("originalHeight") / parseInt($shape.css("height"))) : 1,
                    ...
                    "original-text-width" : parseInt($shape.find(".shapert-clipping").css('width')),
                    "original-text-height" : parseInt($shape.find(".shapert-clipping").css('height'))
                }
                ...
                var percentageClass = $shape.hasClass("percentage")?" percentage":"";
                var pinClasses = "";
                if($shape.hasClass("pin")){
                	pinClasses = " pin";
                	$shape.removeClass("pin");
	                var classList = $shape.attr('class').split(/\s+/);
	                $.each(classList, function(index, item) {
	                    if (item.startsWith("vpin") || item.startsWith("hpin")) {
	                        pinClasses += " "+item;
	                        $shape.removeClass(item);
	                    }
	                });
                }

                $(mainDiv).attr("class", "shapewrapper " + "shapewrapper-"+jimUtil.getBaseID($shape.prop("id"))+autofitClass+percentageClass+pinClasses);
                ...
                var style = {
                    "attributes" : {
                        ...
                        "text-width" : parseInt($shape.find(".shapert-clipping").css('width')),
                        "text-height" : parseInt($shape.find(".shapert-clipping").css('height')),
                ...
                shapeSVG.css2svg["original-text-width"] = parseInt($shape.find(".shapert-clipping").css('width'));
                shapeSVG.css2svg["original-text-height"] = parseInt($shape.find(".shapert-clipping").css('height'));

                shapeSVG.css2svg["original-text-top"] = parseInt(shapeSVG.css2svg["original-text-top"] / shapeSVG.css2svg["original-height-ratio"]);
                shapeSVG.css2svg["original-text-left"] = parseInt(shapeSVG.css2svg["original-text-left"] / shapeSVG.css2svg["original-width-ratio"]);
                shapeSVG.css2svg["original-text-width"] = parseInt(shapeSVG.css2svg["original-text-width"] / shapeSVG.css2svg["original-width-ratio"]);
                shapeSVG.css2svg["original-text-height"] = parseInt(shapeSVG.css2svg["original-text-height"] / shapeSVG.css2svg["original-height-ratio"]);
    ...
    "updateStyle" : function(shapeSVG, style, shapePath) {
        ...
        "updateMargin" : function(shape) {
        	var boxShadow =  $(shape).shapeStyle('box-shadow');
        	$(shape.svg).css("padding", $(shape.shapewrapper).css("padding"));
        	
        	// Update svg
        	var width = (boxShadow === undefined  || boxShadow === null || boxShadow === "none") ? "100%" : $(shape.svg).css("width");
        	var height = (boxShadow === undefined  || boxShadow === null || boxShadow === "none") ? "100%" : $(shape.svg).css("height");
        	$(shape.svg).css("width", "calc(" + width + " - " + (parseInt($(shape.shapewrapper).css("padding-left")) + parseInt($(shape.shapewrapper).css("padding-right"))) + "px)");
        	$(shape.svg).css("height", "calc(" + height + " - " + (parseInt($(shape.shapewrapper).css("padding-top")) + parseInt($(shape.shapewrapper).css("padding-bottom"))) + "px)");
        	
        	// Update textbox
        	var $content = $(shape.shapewrapper).find(".shapert-clipping");
        	 
        	$content.css("left", parseInt($content.css("left")) + parseInt($(shape.shapewrapper).css("padding-left")));
        	$content.css("top", parseInt($content.css("top")) + parseInt($(shape.shapewrapper).css("padding-top")));
        },
        ...
        "updateSize" : function(shapeSVG, oldSize) {
            ...
            if($(shapeSVG.shapewrapper).hasClass("autofit")) {
            ...
            this._getImgSize(sImage, function(imageSize) {
                ...
                if(bgSize!=null && bgSize=="cover"){
                  ...
                  if(vAlign === "50%")
                    ...
                    vString = "YMax";
                  imageObj.setAttribute("preserveAspectRatio",hString.concat(vString).concat(" slice"));
                } else{
     ...
     var VMLEngine = {
        ...
        "createVMLShape" : function($shape) {
            ...
            if (shapeVML != null) {
                ...
                $(mainDiv).attr("class", "shapewrapper " + "shapewrapper-"+jimUtil.getBaseID($shape.prop("id"))+autofitClass+pinClasses);
                $(mainDiv).data("width", $shape.data('width'));
                $(mainDiv).data("widthUnit", $shape.data('widthUnit'));
                $(mainDiv).data("height", $shape.data('height'));
                $(mainDiv).data("heightUnit", $shape.data('heightUnit'));
