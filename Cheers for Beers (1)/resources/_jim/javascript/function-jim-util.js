(function( window, undefined) {
  ...
  var jimUtil = {
    ...
    "animateFitToScreen": function(size) {
      ...
      //Use CSS3 when these browsers are no longer supported
  	  if (jimUtil.fitted && !window.jimDevice.isMobile()) {
      ...
      "getRotatedRectangle" : function(x,y,width,height,angleDegrees){
	   ...
	   "getCanvasAlignmentBox" : function (elem){
			function getCanvasAlignmentBoxRecurse(e){
				...
				if(closest.parent().hasClass("master"))
      ...
      "resizeTable": function($table, newTableWidth, newTableHeight, effect, callback) {
         ...
         if(jimUtil.exists($table) && ($table.is(".table") || $table.is(".datalist") || $table.is(".datagrid"))){
              ...
              if(!jimUtil.exists(newTableWidth) || isNaN(parseInt(newTableWidth, 10)))
              	newTableWidth = $table[0].getBoundingClientRect().width;
              else
              ...
              if(!jimUtil.exists(newTableHeight) || isNaN(parseInt(newTableHeight, 10)))
                newTableHeight = $table[0].getBoundingClientRect().height;
              else
              ...
              for(i=0, iLen = $cells.length; i < iLen; i += 1) {
                ...
                var cellPaddingLeftWidth = isNaN(jimEvent.fn.getCurrentStyle('padding-left',$currentCell)) ? 0 : jimEvent.fn.getCurrentStyle('padding-left',$currentCell);
                var cellPaddingRightWidth = isNaN(jimEvent.fn.getCurrentStyle('padding-right',$currentCell)) ? 0 : jimEvent.fn.getCurrentStyle('padding-right',$currentCell);
                var cellPaddingTopWidth = isNaN(jimEvent.fn.getCurrentStyle('padding-top',$currentCell)) ? 0 : jimEvent.fn.getCurrentStyle('padding-top',$currentCell);
                var cellPaddingBottomWidth = isNaN(jimEvent.fn.getCurrentStyle('padding-bottom',$currentCell)) ? 0 : jimEvent.fn.getCurrentStyle('padding-bottom',$currentCell);
		...
		"adaptItemToNewSize" : function ($item, padding, border, isResizeEvent, trueItemHeight /*fix for autofit in layout*/) {
			...
			if (innerItem != null) {			
			  ...
			  if (!$item.hasClass('panel') && !$item.hasClass('image')) {
			    innerItem.css(property + "-top", padding.top + border.top + borderRadius.topLeft / 2);
			    innerItem.css(property + "-left", padding.left + border.left + borderRadius.topLeft / 2);
			    innerItem.css(property + "-right", padding.right + border.right + borderRadius.topRight / 2);
			    innerItem.css(property + "-bottom", padding.bottom + border.bottom + borderRadius.bottomLeft / 2);
			  }
			}
		},
  ...
  function wrapHorizontalLayoutObject(value, update) {
	...
	if (width != undefined) {
	  ...
	  maxWidth = Math.max(maxWidth, /* tallest children */ (childs.size() > 0) ? childs.map(function(index, x) {return $(x).outerWidth();}).toArray().reduce(function(sum, value) {return Math.max(value, sum);}) : 0);
  ...
  function wrapVerticalLayoutObject(value, show) {
	...
	maxHeight = Math.max(maxHeight, /* tallest children */ childs.map(function(index, x) {return $(x).outerHeight();}).toArray().reduce(function(sum, value) {return Math.max(value, sum);}));
