...
(function(window, undefined) {
  ...
  jQuery.extend(jimEvent.fn, {
    ...
    "jimResize": function(args, callback) {
      if(jimUtil.exists(args)) {
        var self = this, $targets, $target,$parent, type, i, iLen, width, height, bShape, shapeStyle, percentageWidth, percentageHeight, parentWidth, parentHeight;
        if(args.target) {
          ...
          if($targets) {
            for(i=0, iLen = $targets.length; i < iLen; i += 1) {
              ...
              else $target.removeClass("percentage");
...
//                  	height = (height * 100 / parentHeight) +"%";
                ...
                if(args.effect) {
                  ...
                  if($target.jimGetType() === itemType.panel && $target.hasClass("percentage"))
